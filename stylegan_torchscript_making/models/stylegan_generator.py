...
_WSCALE_GAIN = float(torch.sqrt(torch.tensor([2])))
...
class StyleGANGenerator(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_layers = int(torch.log2(torch.tensor([self.resolution // self.init_res]) * 2)) * 2
        ...
        self.tensortoimage = TensorToImage_ARGBModule()
    ...
    def forward(self,
        ...
        OutputImage = self.tensortoimage(synthesis_results['image'])
        return OutputImage

class TensorToImage_ARGBModule(nn.Module):
    """This module is called after SynthesisModule.
    
    Transform synthesis_results(tensor) into image(ARGB).
    """
    def __init__(self):
        super().__init__()
        
    def forward(self, synthesis_results):
        torch.min(synthesis_results)
        min_val = torch.min(synthesis_results)
        max_val = torch.max(synthesis_results)
        OutputImage = synthesis_results.reshape(3,1024,1024)
        OutputImage = (OutputImage - min_val) * 255 / (max_val - min_val)
        OutputImage = torch.clip(torch.add(OutputImage, 0.5), 0, 255).to(torch.int32)
        OutputImage=torch.tensor([0xff])<<24 | OutputImage[0]<<16 | OutputImage[1]<<8 | OutputImage[2]
        
        return OutputImage.to(torch.int32)
...
class TruncationModule(nn.Module):
    ...
    def forward(self, w, trunc_psi=None, trunc_layers=None):
        ...
        if trunc_psi < 1.0 and trunc_layers > 0:
            ...
            layer_idx = torch.arange(self.num_layers).reshape((1, -1, 1))
            #coefs = np.ones_like(layer_idx, dtype=np.float32)
            coefs = torch.ones_like(layer_idx, dtype=torch.float)
...
class SynthesisModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_res_log2 = int(torch.log2(torch.tensor([self.init_res])))
        self.resolution = resolution
        #self.final_res_log2 = int(np.log2(self.resolution))
        self.final_res_log2 = int(torch.log2(torch.tensor([self.resolution])))
        self.w_space_dim = w_space_dim
        self.image_channels = image_channels
        self.final_tanh = final_tanh
        self.const_input = const_input
...
"""
Could not export Python function call 'Blur'. Remove calls to Python functions before export.
Did you forget to add @script or @script_method annotation? 
If this is a nn.ModuleList, add it to __constants__:
    
I soloved this error by Blur{y = F.conv2d(...)} include BlurLayer
"""
"""
...
class BlurLayer(nn.Module):
    ...
    def __init__(self,
        ...
        kernel = torch.tensor([[[[0.0625 ,0.125  ,0.0625],
                                 [0.125  ,0.25   ,0.125 ],
                                 [0.0625 ,0.125  ,0.0625]]]], dtype=torch.float)
        kernel = torch.tile(kernel, (channels, 1, 1, 1))
...
class StyleModLayer(nn.Module):
    ...
    def __init__(self,
        ...
        wscale = _STYLEMOD_WSCALE_GAIN / float(torch.sqrt(torch.tensor(self.w_space_dim)))
        if use_wscale:
    ...
    def forward_style(self, w):
        ...
        if self.space_of_latent == 'w':
            if w.ndim != 2 or w.shape[1] != self.w_space_dim:
                raise ValueError('The input tensor should be with shape '
...
class ConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        wscale = wscale_gain / float(torch.sqrt(torch.tensor(fan_in)))
        if use_wscale:
...
class DenseBlock(nn.Module):
    ...
    def __init__(self,
        ...
        wscale = wscale_gain / float(torch.sqrt(torch.tensor(in_channels)))
        if use_wscale:
            self.weight = nn.Parameter(torch.randn(*weight_shape) / lr_mul)
            self.wscale = wscale * lr_mul
        else:
